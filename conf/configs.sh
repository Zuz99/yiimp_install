...
server
{
    ...
    listen [::]:443 ssl http2;
    server_name '"$1"' '"$2"'.'"$1"';
    # Yiimp default webroot deployed to /var/www
    root /var/www;
    index index.php;
    access_log /var/www/log/'"$1"'.app-access.log;
    error_log  /var/www/log/'"$1"'.app-error.log;
...
function getserverconfig
{
    ...
    local admin_user="${ADMIN_UI_USER:-yiimpadmin}"
    local admin_pass="${ADMIN_UI_PASS:-set-a-password}"

    sudo mkdir -p /etc/yiimp >/dev/null 2>&1

    # YiiMP source expects serverconfig here:
    #   require_once('/etc/yiimp/serverconfig.php');
    sudo tee /etc/yiimp/serverconfig.php >/dev/null <<PHP
<?php
...
define('YIIMP_DBNAME', 'yiimpfrontend');
define('YIIMP_DBUSER', 'panel');
define('YIIMP_DBPASSWORD', '${dbpass}');

define('YIIMP_SITE_URL', '${site_url}');
...
define('YIIMP_ADMIN_EMAIL', '${admin_email}');
define('YIIMP_ADMIN_USER', '${admin_user}');
define('YIIMP_ADMIN_PASS', '${admin_pass}');
define('YIIMP_ADMIN_IP', '${admin_ip}'); // samples: "80.236.118.26,90.234.221.11" or "10.0.0.1/8"
...
PHP
    ...
    # Optional: compatibility for older installer paths
    sudo ln -sfn /etc/yiimp/serverconfig.php /var/www/serverconfig.php >/dev/null 2>&1 || true

    log_message "Generated Yiimp serverconfig.php (/etc/yiimp/serverconfig.php)"
}
