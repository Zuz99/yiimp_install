...
clear
    ...
    sudo mkdir -p /var/www/log/ >/dev/null 2>&1
    # Keep backward-compatible path /var/log/yiimp -> /var/www/log
    sudo ln -sfn /var/www/log /var/log/yiimp >/dev/null 2>&1 || true
    sudo chgrp ${whoami} /var/www/log
    sudo chown ${whoami} /var/www/log
    sudo touch /var/www/log/debug.log
    sudo chown -R www-data:www-data /var/www/log
    sudo chmod -R 775 /var/www/log
    sudo chmod 664 /var/www/log/debug.log

    sudo chgrp www-data /var/www/log -R
    sudo chmod 775 /var/www/log -R
    ...
    if [[ "$RESUME_MODE" == "true" ]]; then
		...
		if [[ -f ${absolutepath}/yiimp/web/index.php ]]; then
			...
			sudo sed -i "/^<?php/a require_once('/etc/yiimp/serverconfig.php');" ${absolutepath}/yiimp/web/index.php
		fi

        log_message "Modified Yiimp configuration files"

        URLREPLACEWEBVAR=/var/www
        ...
        sudo cp -a ${absolutepath}/yiimp/web/. /var/www/
        # Compatibility symlink (some tools/scripts may still reference /var/web)
        sudo ln -sfn /var/www /var/web

		# Copy Yiimp bin scripts to YIIMP_BIN (/var/www/bin)
		sudo mkdir -p /var/www/bin
		sudo cp -a ${absolutepath}/yiimp/bin/. /var/www/bin/
		sudo chmod -R 755 /var/www/bin
		# Align bin/yiimp with /var/www webroot (no trailing /web directory)
		sudo sed -i "s|ROOTDIR=/data/yiimp|ROOTDIR=/var/www|g" /var/www/bin/yiimp >/dev/null 2>&1 || true
        sudo sed -i 's|cd "$ROOTDIR/web"|cd "$ROOTDIR"|g' /var/www/bin/yiimp >/dev/null 2>&1 || true
		log_message "Copied yiimp/bin to /var/www/bin"
        ...
        # sudo cp -r ${absolutepath}/${nameofinstall}/conf/yaamp.php /var/www/yaamp/core/functions
        ...
        # Yiimp web is deployed to /var/www (no per-domain document root directory required)
...
echo -e "$GREEN Done...$COL_RESET"
   ...
   # sudo rm -f "${absolutepath}/${installtoserver}/resume/install_params.conf" >/dev/null 2>&1
    ...
    sudo mkdir -p /var/www/crons/ >/dev/null 2>&1
    sudo chgrp ${whoami} /var/www/crons
    sudo chown ${whoami} /var/www/crons
    sudo cp -r ${absolutepath}/${nameofinstall}/utils/main.sh /var/www/crons/
    sudo chmod +x /var/www/crons/main.sh
    sudo cp -r ${absolutepath}/${nameofinstall}/utils/loop2.sh /var/www/crons/
    sudo chmod +x /var/www/crons/loop2.sh
    sudo cp -r ${absolutepath}/${nameofinstall}/utils/blocks.sh /var/www/crons/
    sudo chmod +x /var/www/crons/blocks.sh
    ...
    sudo sed -i 's/"service $webserver start"/"sudo service $webserver start"/g' /var/www/yaamp/modules/thread/CronjobController.php
    sudo sed -i 's/"service nginx stop"/"sudo service nginx stop"/g' /var/www/yaamp/modules/thread/CronjobController.php
    log_message "Fixed CronjobController.php for service commands"

    # Fix error screen main "backup sql frontend"
    sudo sed -i "s|/root/backup|/var/yiimp/sauv|g" /var/www/yaamp/core/backend/system.php
    ...
    sudo sed -i "s|ExplorerController::createUrl|Yii::app()->createUrl|g" /var/www/yaamp/models/db_coinsModel.php
	...
	if [[ -f /var/www/yaamp/modules/site/coin_form.php ]]; then
		sudo sed -i "s#${SEARCHLINECOINID}#${INSERTLINESCOINID}#" /var/www/yaamp/modules/site/coin_form.php
	fi
    ...
    sudo chmod 775 /var/www/yaamp/runtime >/dev/null 2>&1
    sudo chown www-data:www-data /var/www/yaamp/runtime >/dev/null 2>&1
    log_message "Set permissions for /var/www/yaamp/runtime"
